...
class FaersLoaderEngine:
    ...
    def _process_quarter(self, quarter: str, load_type: str):
        ...
        metadata = {
            ...
            "start_timestamp": datetime.now(timezone.utc), "status": "RUNNING"
        }
        ...
        try:
            ...
            primaryids_to_keep = deduplicate_polars(demo_chunks, self.config.processing.staging_format)

            # --- Unified Filtering and Loading ---
            final_staged_files = self._filter_staged_files_polars(
                staged_chunk_files, primaryids_to_keep, self.config.processing.staging_format
            )

            data_sources = {
                table: path for table, path in final_staged_files.items()
            ...
            self.db_loader.update_load_history(metadata)
            if zip_path and zip_path.exists(): zip_path.unlink()
            if staging_dir and staging_dir.exists(): shutil.rmtree(staging_dir)

    def _parse_quarter_zip(self, zip_path: Path, extract_dir: Path) -> (Generator[Dict[str, Any], None, None], Set[str]):
    ...
    def _filter_staged_files_polars(
        self, staged_files: Dict[str, List[Path]], primaryids_to_keep: Set[str], format: str
    ) -> Dict[str, Path]:
        """Filters staged files using Polars for efficiency."""
        logger.info(f"Filtering staged files to keep {len(primaryids_to_keep)} records using Polars.")
        final_files = {}
        ...
        for table_name, chunks in staged_files.items():
            ...
            if format == 'parquet':
                ...
                lazy_frames = [pl.scan_csv(chunk, separator="$", has_header=True, ignore_errors=True) for chunk in chunks]
            ...
            if filtered_df.height > 0:
                ...
                if format == 'csv':
                    model_headers = [f.lower() for f in FAERS_TABLE_MODELS[table_name].model_fields.keys()]
